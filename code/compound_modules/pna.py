...
from compound_modules.models import AtomEncoder, BondEncoder
...
class PNAGNN(nn.Module):
    ...
    def forward(self, graph: dgl.DGLGraph):
        ...
        #####
        """ raw """
        # graph.ndata['feat']: (# of nodes, 9)
        # graph.edata['feat']: (# of edges, 4)
        
        """ embeded """
        # graph.ndata['feat']: (# of nodes, 256)
        # graph.edata['feat']: (# of nodes, 256)
       
        ####
        #print(f"[PNAGNN] graph ndata: {graph.ndata['feat'].size()}")
        #print(f"[PNAGNN] graph ndata: {graph.edata['feat'].size()}")
...
class PNALayer(nn.Module):
    ...
    def forward(self, g):
        #############################
        # g.ndata: (# of nodes, 256)
        #############################
        h = g.ndata['feat'] 
        h_in = h
        
        ############################
        # edge embeddings: MLP input dim: indim: (2 * 256 + 256): 768
        ############################
        # pretransformation
        #print("self.pretrans_edges")
        
        g.apply_edges(self.pretrans_edges)
        #print("self.pretrans_edges eeeeeeeee")
        
        # aggregation
        #print("self.pretrans_edges hhhhhhhh")
        
        g.update_all(self.message_func, self.reduce_func)
        h = torch.cat([h, g.ndata['feat']], dim=-1)
        
        #print("self.pretrans_edges zzzzzzzzzz")
        
        ############################
        # node embeddings: MLP input dim: ((4 * 3) + 1) * 256: 3328
        ############################
        # post-transformation
        h = self.posttrans(h)
        #print("self.pretrans_edges kkkkkkkkkkkk")
    ...
    def pretrans_edges(self, edges) -> Dict[str, torch.Tensor]:
        ...
        if self.edge_features and self.pairwise_distances:
            ...
            z2 = torch.cat([edges.src['feat'], edges.dst['feat'], edges.data['feat']], dim=-1) # (B, 768)
        else:
