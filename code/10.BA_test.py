...
def main():
    ...
    for idx in range(5):
        ...
        print(f"[Test] RMSE: {TestLoss['RMSE']:.3f}, PCC: {TestLoss['PCC']:.3f}")
        fwrite(open(f"{config['Path']['test_results_path']}/CV{idx}/Test_results.tsv", "w"), Training_Interactions_IDs[test_index], predictions, Training_BA_labels[test_index])
        
        # CASF2016
        TestLoss, predictions, labels = trainer.DTATest(CASF2016_Loader)
        print(f"[CASF2016] RMSE: {TestLoss['RMSE']:.3f}, PCC: {TestLoss['PCC']:.3f}")
        fwrite(open(f"{config['Path']['test_results_path']}/CV{idx}/CASF2016_results.tsv", "w"), CASF2016_Interactions_IDs, predictions, CASF2016_BA_labels)
        
        # CASF2013
        TestLoss, predictions, labels = trainer.DTATest(CASF2013_Loader)
        print(f"[CASF2013] RMSE: {TestLoss['RMSE']:.3f}, PCC: {TestLoss['PCC']:.3f}")
        fwrite(open(f"{config['Path']['test_results_path']}/CV{idx}/CASF2013_results.tsv", "w"), CASF2013_Interactions_IDs, predictions, CASF2013_BA_labels)
        
        # CSAR2014
        TestLoss, predictions, labels = trainer.DTATest(CSAR2014_Loader)
        print(f"[CSAR2014] RMSE: {TestLoss['RMSE']:.3f}, PCC: {TestLoss['PCC']:.3f}")
        fwrite(open(f"{config['Path']['test_results_path']}/CV{idx}/CSAR2014_results.tsv", "w"), CSAR2014_Interactions_IDs, predictions, CSAR2014_BA_labels)
        
        # CSAR2012
        TestLoss, predictions, labels = trainer.DTATest(CSAR2012_Loader)
        print(f"[CSAR2012] RMSE: {TestLoss['RMSE']:.3f}, PCC: {TestLoss['PCC']:.3f}")
        fwrite(open(f"{config['Path']['test_results_path']}/CV{idx}/CSAR2012_results.tsv", "w"), CSAR2012_Interactions_IDs, predictions, CSAR2012_BA_labels)

        # CSARset1
        TestLoss, predictions, labels = trainer.DTATest(CSARset1_Loader)
        print(f"[CSARset1] RMSE: {TestLoss['RMSE']:.3f}, PCC: {TestLoss['PCC']:.3f}")
        fwrite(open(f"{config['Path']['test_results_path']}/CV{idx}/CSARset1_results.tsv", "w"), CSARset1_Interactions_IDs, predictions, CSARset1_BA_labels)
        
        # CSARset2
        TestLoss, predictions, labels = trainer.DTATest(CSARset2_Loader)
        print(f"[CSARset2] RMSE: {TestLoss['RMSE']:.3f}, PCC: {TestLoss['PCC']:.3f}")
        fwrite(open(f"{config['Path']['test_results_path']}/CV{idx}/CSARset2_results.tsv", "w"), CSARset2_Interactions_IDs, predictions, CSARset2_BA_labels)

        # Astex
        TestLoss, predictions, labels = trainer.DTATest(Astex_Loader)
        print(f"[Astex] RMSE: {TestLoss['RMSE']:.3f}, PCC: {TestLoss['PCC']:.3f}")
        fwrite(open(f"{config['Path']['test_results_path']}/CV{idx}/Astex_results.tsv", "w"), Astex_Interactions_IDs, predictions, Astex_BA_labels)
        print()
...
def fwrite(fw, interaction_ids, predictions, labels):
    ...
    for ids, prediction, label in zip(interaction_ids, predictions, labels):
        fw.write(f"{ids.split('_')[0]}\t{ids.split('_')[2]}\t{prediction:.3f}\t{label:.2f}\n")
