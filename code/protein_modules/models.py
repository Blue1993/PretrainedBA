...
from protein_modules.base_layers import BertCrossattLayer, BertIntermediate, BertOutput 
...
class PocketConvLayer(nn.Module):
    def __init__(self, config, dim_in, dim_out, padding = 1, stride = 1):
        ...
        self.config = config
        dim_in_tuple = (1024, self.config["Architecture"]["hidden_size"], self.config["Architecture"]["hidden_size"], 
                            self.config["Architecture"]["hidden_size"], self.config["Architecture"]["hidden_size"])
        dim_out_tuple = (self.config["Architecture"]["hidden_size"], self.config["Architecture"]["hidden_size"], 
                            self.config["Architecture"]["hidden_size"], self.config["Architecture"]["hidden_size"], 
        ...
        for idx, dilation_rate in enumerate(dilation_tuple):
            self.first_.append(PocketEncoderLayer(self.config, dim_in = dim_in_tuple[idx], dim_out = dim_out_tuple[idx], 
                kernel_size = 3, padding = dilation_rate, dilation = dilation_rate, stride = 1))

        for idx, dilation_rate in enumerate(dilation_tuple):
            self.second_.append(PocketEncoderLayer(self.config, dim_in = dim_in_tuple[idx], dim_out = dim_out_tuple[idx], 
                kernel_size = 5, padding = 2 * dilation_rate, dilation = dilation_rate, stride = 1))

        for idx, dilation_rate in enumerate(dilation_tuple):
            self.third_.append(PocketEncoderLayer(self.config, dim_in = dim_in_tuple[idx], dim_out = dim_out_tuple[idx], 
                kernel_size = 7, padding = 3 * dilation_rate , dilation = dilation_rate, stride = 1)) 
    ...
    def forward(self, query_embeddings, aa_embeddings, attention_mask):    
        ...
        aa_embeddings = first_aa_embeddings
        
        return aa_embeddings.transpose(1, 2), None
...
class Pseq2Sites(nn.Module):
    def __init__(self, config):
        ...
        # Query Embedding
        self.position_embeddings = nn.Embedding(config["Architecture"]["max_lengths"], 1024) # (Max_position, Hidden_size)
        self.token_type_embeddings = nn.Embedding(13, 1024, padding_idx=0)
        self.LayerNorm = BertLayerNorm(1024, eps=1e-12)

        self.protein_position_embeddings = nn.Embedding(config["Architecture"]["max_lengths"], 256)
        ...
        self.protein_features = nn.Linear(1024, self.config["Architecture"]["hidden_size"])

    def forward(self, prots, total_prots_data, attention_mask, position_ids):
    
        feats = deepcopy(prots) # (B, L, H) [H: 1024]

        position_embeddings = self.position_embeddings(position_ids) # (B, L, H) [H: 1024]

        protein_position_embeddings = self.protein_position_embeddings(position_ids) # (B, L, H') [H': 256]

        # input features
        feats = feats + position_embeddings
        feats = self.LayerNorm(feats) # (B, L, H) [H: 1024]
        feats = self.dropout(feats) # (B, L, H) [H: 1024]
        
        # protein features
        prot_feats = self.LayerNorm2(total_prots_data) # (B, L, H) [H: 1024]
        prot_feats = self.protein_features(prot_feats) # (B, L, H') [H': 256]
        prot_feats = self.dropout(prot_feats) # (B, L, H') [H': 256]

        # query features
        query_embeddings = prot_feats + protein_position_embeddings # (B, L, H') [H': 256]
        query_embeddings = self.LayerNorm3(query_embeddings) # (B, L, H') [H': 256]
        query_embeddings = self.dropout(query_embeddings) # (B, L, H') [H': 256]
        ...
        return feats, prot_feats, outputs, att_probs

class ProtsEncoder(nn.Module):
    def __init__(self, config):
        super(ProtsEncoder, self).__init__()
    ...
    def forward(self, prots, total_prots_data, attention_mask, position_ids):
        ...
        return feats, prots_extended_attention_mask
